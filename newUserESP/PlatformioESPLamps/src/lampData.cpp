...
void setRgbColors(byte arr[30 /*numLeds*/][4 /*colorCount*/], byte valueToPut[4 /*colorCount*/]) { //iestata 2d krasu masiva atbilstosas krasu vertibas
  bool breakOut = false;
  ...
  for (int i = 30 /*numLeds*/; i > startIn; i--) { //pabida vertibas uz labo pusi lidz startIn+1, jo startIn vertibu aiztikt nevar ta iemesla del, ka tad var par vienu vienibu pabidities nepareiza vertiba ta, kas atradas ieks startIn ieprieks
  ...
  if (divide) {//ja 2d vertiba, kas ievadita 2d masiva atradas pa vidu esosai
    for (int i = 0; i < 4 /*colorCount*/; i++) {
  ...
  while (breakCheck) {
    for (int i = startIn + startFreeSpace; i < 30 /*numLeds*/; i++) {
      ...
      if (zeroCount == 4 /*colorCount*/) {//ja visas masiva krasu vertibas bija nulle
        countFreeSpaces++;
      } else if (countFreeSpaces > 0) {
        break;
      }
      if (i == 30 /*numLeds*/ - 1) {//ja visas veribas pec pedejas aizpilditas sunas ir nulle tad funkcija beidz savu darbibu
        breakCheck = false;
        break;
      }
    }
  ...
  for (int i = 0; i < 30 /*numLeds*/; i++) { //pedeja parbaude, kas nosaka to vai pretim 2d dalai, kuras vertiba ir nulle atrodas 1d suna, kuras vertiba lielaka par nulli
...
void setValueInArr(uint32_t arr[30 /*numLeds*/], uint32_t valueToPut) {//ievieto ievadito vertibu atbilstosaja 1d masiva atkariba no esosa 2d masiva
  bool breakOut = false;
...
void displayAdrColors(int startEnd[2], byte colorToDisplay[4 /*colorCount*/]) { //iestata krasas spidesanu kadam gabalam adresejamam diodem
  String sendOut;
...
void recvWithStartEndMarkers(String inputString) { //datu nolasisanas funkcija
        ...
        switch (countDataSwCase) {
          ...
          case 2://tiek noteikts kuriem led pikseliem ir jastrada
            ...
            do {
              ...
              if (isDigit(rc)) { //ja sanemtais baits ir cipars, tad tas tiek ievietots ieks saraksta talakai apstradei
                receivedChars[ndx] = rc;
                ndx++;
              }
              rc = inputString[x];
              if (rc == adrLedNumDivider) {//ja sanemtais baits ir '-', tad tas nozime, ka tiek sanemti adresejamo diozu lentas dati
                ledPartStartEnd[0] = atoi(receivedChars);
                ndx = 0;
              } else if (rc == ledPartEndMarker) {
                ledPartStartEnd[1] = atoi(receivedChars);
                ndx = 0;
                for ( int i = 0; i < 3;  ++i ) { //notira receivedChars masivu
            ...
            break;
          case 4:
            if (dataType == 1) { //funkcijas numura iestatisana
              funcNum = atoi(receivedChars);
              ndx = 0;
            }
            break;
          case 5:
            if (dataType == 0) { //sarkanas krasas iestatisana
              rgbReceive[0] = atoi(receivedChars);
              ndx = 0;
            }
            break;
          case 7:
            if (dataType == 1) { //funkcijas parametru vertibas iestatisana
              funcPar = atoi(receivedChars);
              ndx = 0;
            }
            break;
          case 8://zalas krasas iestatisana
            if (dataType == 0) { //zalas krasas iestatisana
...
void setNewData() { //sanemto datu apstrade
  for ( int i = 0; i < 3;  ++i ) { //notira receivedChars masivu
    receivedChars[i] = (char)0;
  }
  updateAdrLedPart(ledPartStartEnd);//apdeito diozu dalu masivu
  if (dataType == 0) { //ja ir iestatita krasa, tad atbilstosajas vietas funkcijas parametrs un funkcijas numurs tiks mainits uz nulli
    funcNum = 1;
    funcPar = 1;
  }
  //    Serial.println("a");
  ...
  if (dataType == 0) { //ta pati funkcija, kas setValueInArr, bet paredzeta darbam ar 2d masivu. Krasu iestatisana atbilstosajas sunas
    setRgbColors(rgb, rgbReceive);
    displayAdrColors(ledPartStartEnd, rgbReceive);
  } else { //ja tika iestatita funkcijas vertiba, tad jaatbrivo atbilstosa krasas vieniba
    for (int i = 0; i < 4 /*colorCount*/; i++) { //jo funkcijai setRgbColors nevar padot rgvReceive no ieprieksejas vertibas, vai pointeri uz rgb[startIn]
...
void funcExecute() { //iet cauri funcNum masiva vertibam un atbilstosi izpilda katru funkciju, kas der
  for (int i = 0; i < 30 /*numLeds*/; i++) { //nepieciesams, lai dzitu uz prieksu for ciklu un ta vertibu izmantot jau ka parametru izpildamajai funkcijai
    if (funcNumArr[i] == 0) { //izbeidz funkcijas darbibu, ja funcNum masiva tiek atrasta vertiba 0
      //      Serial.println("Found 0");
      break;
    }
    if (funcNumArr[i] != 1) {
      switch (funcNumArr[i]) { //funkciju numuri sakas no 2 ieskaitot
        case 2: //izpildas solid rainbow
          if (isFirstTime[i]) { //ja funkcija notiek pirmo reizi, tad tiek iestatitas sakuma vertibas
            oldTimeInt[i] = millis();
            solidRainbow(i, 1);
            isFirstTime[i] = 0;
          ...
          break;
        case 3://solid fade
          if (isFirstTime[i]) { //vienu reizi tiek saglabatas augstakas krasu vertibas, kas ari velak tiek feidotas
        ...
        case 4://blink
          ...
          isFirstTime[i] = 0;
          if (millis() - oldTimeInt[i] >= funcParArr[i] || millis() < oldTimeInt[i]) {
            if (blinkOff[i] == true) {//sis globalais buls seko lidzi tam vai gaisma tiek ieslegta vai izslegta
              blinkOff[i] = false;
            } else {
              blinkOff[i] = true;
            }
...
void updateAdrLedPart(int startEnd[2]) {//Jaizveido vertibu pastumsana uz kreiso pusi pec labas masiva puses apstrades
  divide = false;
  startIn = 0;
  for (int i = 0; i < 30 /*numLeds*/; i++) {
    for (int j = 0; j < 2; j++) {
  ...
  for (int i = 0; i < 30 /*numLeds*/; i++) { //atrod startIn vertibu
  ...
  for (int i = 30 /*numLeds*/; i >= startIn; i--) { //"pabida" vertibas uz labo pusi
    for (int x = 0; x < 2; x++) {
  ...
  for (int i = 0; i < 2; i++) {//Ievieto vertibas atbilstosajas dalas
    adrStartEnd[startIn][i] = startEnd[i];
  }

  if (startEnd[0] <= adrStartEnd[startIn - 1][1]) { //masiva pirms ievadita masiva tiek salidzinatas vertibas un ja ta ir mazaka par ievadita masiva sakuma vertibu, tad ta paliek par 1 mazaka
    adrStartEnd[startIn - 1][1] = startEnd[0] - 1;
  }

  if (adrStartEnd[startIn - 1][1] < adrStartEnd[startIn - 1][0]) { //ja izmainu rezultata masiva beigu vertiba ir lielaka par sakuma vertibu, tad masiva vertiba klust par 0
    adrStartEnd[startIn - 1][0] = 0;
    adrStartEnd[startIn - 1][1] = 0;
  }

  bool breakCheck = false;//nepieciesams, lai tiktu ara no galvena lupa
  for (int i = startIn + 1; i < 30 /*numLeds*/; i++) { //parbauda cik talu sniedzas ievadita vertiba masiva pa labi un maina vertibas atbilstosi tam
    for (int x = 0; x < 2; x++) {
  ...
  while (breakCheck) {
    for (int i = startIn + startFreeSpace; i < 30 /*numLeds*/; i++) {
      if (adrStartEnd[i][0] == 0) {
        countFreeSpaces++;
      } else if (countFreeSpaces > 0) {//ja adrStartEnd[i][0]>0, tad iziet ara no dota for cikla
        break;
      }
      if (i == 30 /*numLeds*/ - 1) {//ja visas veribas pec pedejas aizpilditas sunas ir nulle tad funkcija beidz savu darbibu
        breakCheck = false;
  ...
  /*Serial.println("Final: ");
    ...
    for (int j = 0; j < 2; j++) {
      if (j == 0) {
        Serial.println('{');
        Serial.println(adrStartEnd[i][j]);
        Serial.println(',');
      } else {
        Serial.println(adrStartEnd[i][j]);
        Serial.println('}');
